...
| **D138** | **A subscribe is finished by its SUBACK, not by its OK.** `OK` means *accepted* (spec, and `rnwf_at.c`'s own header rule); the result arrives later as `+MQTTSUB:<REASON_CODE>` — 0/1/2 granted QoS, 128 error. Sending the next `AT+MQTTSUB` while one is outstanding gets it refused **by the module** with `ERROR:8.0`, and it never reaches the broker: mosquitto logged one SUBSCRIBE then silence. Which subscribe lost depended on broker latency, so the failure moved between steps and looked non-deterministic. Confirmed by the AT reference's own example and by Microchip's driver, which treats a reason code above QoS 2 as an error. A code above 2 now fails the step rather than passing it | settled |
| **D139** | **A link-local address does not count as "associate and get an IP".** With IPv6 SLAAC the module announces `fe80::` the moment the link is up — observed as `+WSTAAIP:1,"FE80::DF6A:DC30:93BF:60AE"` — so completing on the first `+WSTAAIP` let the broker steps start before DHCPv4 had produced a usable address. `fe80::` is now ignored for step completion and for the reported address; the step's own timeout still bounds the wait | settled |
| **D140** | **Every path that starts the AT client must re-derive its config from settings first.** `module_cfg` borrows *pointers* into `settings` for strings but takes *copies* of the scalars, so after boot `ssid` and `pass` tracked console edits while `sec`, `port` and `keep_alive` silently did not. `set sec 3` therefore never reached the module: it hunted for an *open* network, rejected every encrypted AP (`privacy fail, op_mode 0` in its own MAC log) and timed out. Invisible to `show` and to every diagnostic, because all of them read `settings` rather than what was sent | settled |
| **D141** | **Module firmware on a *running* device is updated with the SDK's `nvm-update.py` over the ordinary AT UART** (`AT+NVMER`, `AT+NVMWR`, `AT+NVMRD`, `AT+OTAVFY`, `AT+OTAACT`), pyserial only — no FTDI, no bit-bang, no D2XX. **ADR-0026's DFU shim remains the path for a device that is not running**, which is what DFU is for; the SDK's own `tools/dfu` is the same `ftd2xx` bit-bang tool that cannot run on macOS | settled |
...
18. ⚠️ Hardware bring-up: `EV10P22A` + `EV72E72A`, four jumpers — TX, RX, MCLR (PB04, D132) and a
    ...
    **Done for the Wi-Fi path** (2026-08-18): `test module` gets `+BOOT` in ~3.9 s, and `test broker`
    completes association, DHCP, broker connect, CONNACK and all three subscribes against a real
    mosquitto. Three faults stood in the way, none visible to any test: D140, D138 and D139.
    Still unproven on hardware: **publishing** and the daemon end to end. The payload bug behind that
    is fixed — the button JSON is quoted text inside a quoted AT argument, and every quoted argument is
    now escaped as the specification requires — but nothing has yet published from the device. Both
    modules were updated to firmware **3.1.0** and then **3.2.0** (ADR-0025), the second time over the
    AT UART rather than by DFU (D141), which is what keeps `rnwf_at_cmds.h` a claim about the silicon
    in front of us rather than about a specification revision it never ran (D133).
...
- **Module firmware gate (pre-ship, ADR-0025):** `probe_rnwf02.py --require-version 3.2.0` on every
