...
18. ⚠️ Hardware bring-up: `EV10P22A` + `EV72E72A`, four jumpers — TX, RX, MCLR (PB04, D132) and a
    ...
    **Done for the Wi-Fi path** (2026-08-18): `test module` gets `+BOOT` in ~3.9 s, and `test broker`
    ...
    Still unproven on hardware: **publishing** (the payload is malformed today — the button JSON is
    quoted text inside a quoted AT argument, unescaped) and the daemon end to end. Both modules were updated to firmware **3.1.0** first (ADR-0025), which is what makes
