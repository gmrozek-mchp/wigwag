...
| D25 | Debug = SWD, pyOCD runner (PICkit 5 / PICkit Basic / Atmel-ICE / J-Link all work) — **verified on the cnano's nEDBG, with DFP ≥ 1.5.437; see D69**. *The product's connector is the 10-pin Cortex Debug header, D140 — a 5-pin ICSP header cannot carry SWD* | settled |
...
| **D137** | **Module pin 11 goes to `PA11`, held tri-state — a test point does not satisfy "do not leave unconnected".** Table 2-1 note 5 says exactly that, and the pin description asks for a tri-stated host I/O or an external switch; `PINOUT.md` had planned a test point, which is electrically still unconnected. `PA11` is the cheapest of the eight free pads: it retires the `PA10`+`PA11` mux-D module-UART swap, which ADR-0023 had already rejected on two independent grounds (ADR-0009 declined to let the module link depend on `VDDIO2`; it discards D76's continuity with the dev board). Any single MVIO pad breaks one pair, `PA17`/`PA18` would cost a lamp standby, and `PA24`/`PA25` are clear on purpose (D126). **19 of 28 pins used, seven free.** Firmware must configure `PA11` as a high-impedance input and never drive it | settled |
| **D138** | **The three FET gates need pulldowns, and item 20's parts list had none.** MCU pins are high-impedance inputs at power-on reset, so without them all three gates float from POR until firmware drives them and the lamps sit indeterminate at boot — the fail-*invisible* behaviour Rule 4 exists to prevent. 100 k gate-to-source, electrically free against a 3.3 V drive. Also fitted: `RESET` gets a 100 nF **DNP** pad (a cap there can upset some debuggers' reset timing, so the pad exists without the part), and `SW1` deliberately keeps **no** external part per item 20 even though it is user-touchable and lands straight on an MCU pin — recorded so a static-discharge failure would not look like an oversight | settled |
| **D139** | **The module's series resistors do three jobs, so they are resistors rather than links.** §2.4's recommended series element (D136), the stuffing option for flow control (D128, on the RTS/CTS pair), and **isolation jumpers**: pulling the two on `SERCOM0` detaches the module from the MCU's UART, so a fully-populated board can still be driven by `fake_rnwf02.py` (ADR-0015) over the D59 breakout. With the module unpopulated the same path works with nothing removed — **the board is fully exercisable without the Wi-Fi module**, which matters because it is the long-lead part | settled |
| **D140** | **Debug is a 10-pin Cortex Debug connector (1.27 mm, keyed) and nothing else — a 5-pin ICSP header cannot carry SWD.** PICkit 5 UG §3.3.1's `Cortex SWD` column puts **`SWDIO` on connector pin 8** and `SWO` on pin 4, and states that a 6-pin header "will result in the loss of functions on Pins 7 and 8 affecting EJTAG, JTAG, SWD and ISP". The 5-pin header first specified would have wired `SWDIO` to the `SWO` position and not worked. The 10-pin is also what the tools ship for: **PICkit Basic includes an 8-pin-to-10-pin Arm SWD adapter board and cable** (§3.3.2), PICkit 5 reaches it via its Debugger Adapter Board (§3.3.3.2.2), and Atmel-ICE and J-Link Mini/EDU use it natively. **Pins 6 and 8 are both left unconnected**: the PL10 is Cortex-M0+ with no ITM so there is no `SWO`, and §3.3.3.2.2 maps `SWO` to 10-pin pin **8** where the ARM standard puts it on pin **6** — the two disagree only on the pin this board does not use | settled |
| **D141** | **The schematic is generated from `SCHEMATIC.md` by [`hardware/gen_schematic.py`](../hardware/gen_schematic.py), and connectivity is by net label rather than drawn wire.** So placement is purely cosmetic — rearranging in Eeschema cannot change the netlist — and a value change is a one-line edit plus a regenerate, not surgery on a 2 000-line S-expression. Symbols are authored in the script from the datasheet pin tables and emitted **both** into the schematic's `lib_symbols` cache and into a real `wigwag.kicad_sym` + `sym-lib-table`, so `lib_id`s resolve instead of raising `lib_symbol_issues` on all 60 parts. UUIDs are hash-derived, so an unchanged netlist regenerates byte-identical and diffs stay meaningful. **Verified on KiCad 10.0.5** (not the 9 the plan assumed): ERC reports **1 violation**, the yet-to-be-drawn module footprint, and KiCad's own netlist export matches the spec — 46 named nets, identical membership, plus 29 deliberately unconnected pins. Every footprint but the module's was checked to exist in the installed libraries with pad names matching the symbol's pin numbers | built |
...
### Phase 3 — PCB (KiCad 10, 4-layer)

*Installed toolchain is **KiCad 10.0.5**, not the 9 this plan was written against. The generated
schematic parses, ERCs and exports cleanly on it (D141).*
...
20. Schematic. **Net-level spec complete: [`hardware/SCHEMATIC.md`](../hardware/SCHEMATIC.md)** —
    every part, every pin, every net, D137–D139. The KiCad file is generated from it, and the
    document stays the artifact of record if the two drift. **One flat sheet on 11 × 17 in
    (ANSI B / US Ledger), landscape** — Page Settings → `B`, `(paper "B")`. D131.
    Remaining: the `.kicad_sch` itself, the LED part choice (which sets the lamp resistors) and the
    USB-C receptacle part (which sets Phase 4's cutout).
    `hardware/PINOUT.md` remains the pin map; the notes below are the parts list and the non-pin
    things, all now folded into `SCHEMATIC.md`.
    - **PL10 SSOP-28** — `VDDIO2` tied to 3V3 alongside `VDD` (§3.2.3, D50); decoupling per
      §40; **10-pin Cortex Debug header** (1.27 mm, keyed — D140; a 5-pin ICSP header cannot carry
      SWD) with a **1 kΩ pull-up on SWCLK** (Table 40-6, not optional) and a weak
